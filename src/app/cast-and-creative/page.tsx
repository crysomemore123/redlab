...
interface TeamMember {
  ...
  imageSrc: string;
  altText: string;
  width: number;  // ADDED: width for Next/Image
  height: number; // ADDED: height for Next/Image
  website?: string;
  websiteText?: string;
}

// Data for the Cast and Creative page with image dimensions
const castAndCreativeData: TeamMember[] = [
  {
    ...
    width: 200, height: 267, // Example dimensions
  ...
  // ... Add width and height to all other members similarly
  // I will fill in the rest with the same placeholder dimensions for brevity
   {
    name: 'ROSS DEGRAW',
              ...
              <div key={member.name} className={styles.teamEntry}>
                <div className={styles.teamImageWrapper}>
                  ...
                  <Image
                    src={member.imageSrc}
                    alt={member.altText}
                    width={member.width}   // Using width from data
                    height={member.height} // Using height from data
                    className={styles.teamImage}
                    priority={index < 3}
                    sizes="(max-width: 768px) 100vw, 200px" // Helps Next.js optimize
                  />
                </div>
        ...
        <div className={styles.teamList}>
          {[...Array(4)].map((_, index) => (
            <div key={index} className={styles.teamEntry} style={{ opacity: 0.5 }}>
              ...
              ></div>
              <div className={styles.teamInfo}>
                <div className={`${styles.skeletonPulse} ${styles.skeletonLine} w-3/4 mb-2`}></div>
                <div className={`${styles.skeletonPulse} ${styles.skeletonLine} w-2/3 mb-2 h-4`}></div>
                <div className="space-y-2">
                  <div className={`${styles.skeletonPulse} ${styles.skeletonLine} w-full h-4`}></div>
                  <div className={`${styles.skeletonPulse} ${styles.skeletonLine} w-5/6 h-4`}></div>
                </div>
              </div>
            </div>
          ))}
